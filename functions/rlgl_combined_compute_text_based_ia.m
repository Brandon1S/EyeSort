function EEG = rlgl_combined_compute_text_based_ia(EEG, txtFilePath, offset, pxPerChar, ...
    ...
    for iEvt = 1:length(EEG.event)
        ...
        if trialRunning && ~isempty(lastValidKey)
            if isKey(boundaryMap, lastValidKey)
                ...
                % Check if the current event is a fixation event.
                % Updated condition: 
                if (startsWith(eventType, 'R_fixation') || (length(eventTypeNoSpace) == 4 && all(isstrprop(eventTypeNoSpace, 'digit'))))
